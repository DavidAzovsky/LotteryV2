...
describe("Lottery", async () => {
  ...
  //   await expect(
  //     lottery.connect(user1).verifiedWhiteListedUser([], user2.address)
  ...
  describe("enter to lottery", async () => {
    ...
    it("whitelisted user should enter successfully", async () => {
      ...
      const proof = abi.encodeParameter("bytes", proofArray);
      await lottery.connect(whiteUser2).enter({ data: proof });
